...
Функция ВыгрузитьФайлВXML(ИмяФайла,ВерсииВсехФайлов)
...
Функция РаспаковатьФайлыПоМаске(Путь,Маска,ИскатьВПодкаталогах)
	...
	НомерФайла = 0;
	КоличествоРаспакованныхФайлов = 0;
	Для Каждого Файл Из Файлы Цикл
		НомерФайла = НомерФайла + 1;
		Лог.Информация("Файл " + НомерФайла + " из " + КоличествоФайлов + ": " + Файл.ПолноеИмя);

		Если ВыгрузитьФайлВXML(Файл.ПолноеИмя,ВерсииВсехФайлов) Тогда
			КоличествоРаспакованныхФайлов = КоличествоРаспакованныхФайлов + 1;
		КонецЕсли;
	КонецЦикла;

	Возврат КоличествоРаспакованныхФайлов;
...
Процедура РазобратьОбработкуИлиОтчетВКаталогеИПодКаталогах(Путь,РежимРаботы)
	...
	КоличествоРаспакованныхФайлов = РаспаковатьФайлыПоМаске(Путь,"*.erf",Истина)
		+ РаспаковатьФайлыПоМаске(Путь,"*.epf",Истина);

	Если РежимРаботы = "CheckDif" И КоличествоРаспакованныхФайлов > 0 Тогда
