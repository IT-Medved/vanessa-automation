...
Функция ПолучитьПамятьПроцессов1С()
	...
	Если Файл.Существует() Тогда
		...
		Текст.Закрыть();
	Иначе
		Возврат Новый Структура("ОбщаяПамятьКБ, СписокПроцессов", 0, "Файл с данными не создан");
	КонецЕсли;
	...
	Попытка
		...
		Для каждого Стр Из МассивСтрок Цикл
			...
			Если МассивПолей.Количество() >= 5 Тогда
				...
				ПамятьКБ = 0;
				Попытка
					ПамятьКБ = Число(ПамятьСтр) / 1024 / 1024;
				Исключение
					Продолжить;
				КонецПопытки;
				
				ОбщаяПамятьКБ = ОбщаяПамятьКБ + ПамятьКБ;
				СписокПроцессов = СписокПроцессов + "  " + ИмяПроцесса + " (PID:" + PID + "): " + Формат(ПамятьКБ, "ЧЦ=10; ЧДЦ=0") + " mb" + Символы.ПС;
			КонецЕсли;
		КонецЦикла;
		
		Возврат Новый Структура("ОбщаяПамятьКБ, СписокПроцессов", ОбщаяПамятьКБ, СписокПроцессов);
		
	Исключение
		Возврат Новый Структура("ОбщаяПамятьКБ, СписокПроцессов", 0, "Ошибка получения информации о памяти: " + ОписаниеОшибки());
	КонецПопытки;
КонецФункции

Процедура ВывестиИнформациюОПамятиПроцессов1С()
	Инфо = ПолучитьПамятьПроцессов1С();
	Если Инфо.ОбщаяПамятьКБ > 0 Тогда
		ОбщаяПамятьМБ = Инфо.ОбщаяПамятьКБ;
