...
Функция ПолучитьПамятьПроцессов1С()
	// Получаем информацию о процессах 1С в Windows
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("txt"); 
	СтрокаКоманды = "wmic process where (name=""1cv8.exe"" or name=""1cv8c.exe"" or name=""1cv8s.exe"") get Name,ProcessId,SessionId,WorkingSetSize /format:csv > """ + ИмяВременногоФайла + """ 2>&1";
	
	//ИмяВременногоBAT = ПолучитьИмяВременногоФайла("bat");
	//ЗТ = Новый ЗаписьТекста(ИмяВременногоBAT,"UTF-8",,Истина);
	//ЗТ.ЗаписатьСтроку("echo on");
	////ЗТ.ЗаписатьСтроку("chcp 65001");
	//ЗТ.ЗаписатьСтроку(СтрокаКоманды);
	//ЗТ.Закрыть();
	//
	//retCode = -1;
	//ЗапуститьПриложение(ИмяВременногоBAT,,Истина,retCode);
	...
	Если Файл.Существует() Тогда
		...
		Текст.Закрыть();
	Иначе
		Возврат Новый Структура("ОбщаяПамятьМБ, СписокПроцессов", 0, "Файл с данными не создан");
	КонецЕсли;
	
	Попытка
		// Парсим CSV вывод wmic
		// Пример формата:
		// Node,Name,ProcessId,SessionId,WorkingSetSize
		// ИмяПК,1cv8.exe,92676,1,633987072
		// ИмяПК,1cv8c.exe,93176,1,258809856
		...
		Для каждого Стр Из МассивСтрок Цикл
			...
			Если Не ЗаголовокОбработан Тогда
				Если ВРег(СокрЛП(Стр)) = "node,name,processid,sessionid,workingsetsize" Тогда
					ЗаголовокОбработан = Истина;
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			
			// Разбиваем по запятой (CSV формат wmic)
			МассивПолей = СтрРазделить(Стр, ",", Истина);
			
			Если МассивПолей.Количество() >= 5 Тогда
				ИмяПроцесса = МассивПолей[1]; // Name
				PID = МассивПолей[2];         // ProcessId
				ПамятьСтр = МассивПолей[4];   // WorkingSetSize (в байтах)
				
				// Преобразуем память из байтов в КБ
				ПамятьМБ = 0;
				Попытка
					ПамятьМБ = Число(ПамятьСтр) / 1024 / 1024;
				Исключение
					Продолжить;
				КонецПопытки;
				
				Если ПамятьМБ > ПорогПамятиМБ Тогда
					Сообщить("Превышение порога памяти: PID=" + PID + ", " + Формат(ПамятьМБ, "ЧЦ=10; ЧДЦ=0"));
				КонецЕсли;	
				
				ОбщаяПамятьМБ = ОбщаяПамятьМБ + ПамятьМБ;
				СписокПроцессов = СписокПроцессов + "  " + ИмяПроцесса + " (PID:" + PID + "): " + Формат(ПамятьМБ, "ЧЦ=10; ЧДЦ=0") + " mb" + Символы.ПС;
			КонецЕсли;
		КонецЦикла;
		
		Возврат Новый Структура("ОбщаяПамятьМБ, СписокПроцессов", ОбщаяПамятьМБ, СписокПроцессов);
		
	Исключение
		Возврат Новый Структура("ОбщаяПамятьМБ, СписокПроцессов", 0, "Ошибка получения информации о памяти: " + ОписаниеОшибки());
	КонецПопытки;
КонецФункции

Процедура ВывестиИнформациюОПамятиПроцессов1С()
	Инфо = ПолучитьПамятьПроцессов1С();
	Если Инфо.ОбщаяПамятьМБ > 0 Тогда
		ОбщаяПамятьМБ = Инфо.ОбщаяПамятьМБ;
		Лог.Информация("=== Память процессов 1С: " + Формат(ОбщаяПамятьМБ, "ЧЦ=10; ЧДЦ=0") + " mb ===");
		Если Не ПустаяСтрока(Инфо.СписокПроцессов) Тогда
			Сообщить(Инфо.СписокПроцессов);
		КонецЕсли;
	Иначе
		Если Не ПустаяСтрока(Инфо.СписокПроцессов) И Лев(Инфо.СписокПроцессов, 1) <> " " Тогда
			Сообщить(Инфо.СписокПроцессов);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыполнитьЗапускОднойСборки(ПараметрыСборки)
	...
	Сообщить("Строка запуска сборки: " + СтрокаЗапускаСборки);
	...
	ДатаДляВыводаПамятиПроцессов = ТекущаяДата();
	ПериодВыводаДанныхОПамятиПроцессов = 120;
	...
	Пока Истина Цикл
		...
		Если ЗначениеЗаполнено(ИмяФайлаЛога) И НадоЧитатьЛог Тогда
			Sleep(10000);
			// Периодически выводим информацию о памяти процессов 1С
		Иначе	
			Sleep(2000);
		КонецЕсли;
		
		Если (ТекущаяДата() - ДатаДляВыводаПамятиПроцессов) >= ПериодВыводаДанныхОПамятиПроцессов Тогда
			ДатаДляВыводаПамятиПроцессов = ТекущаяДата();
			Попытка
				ВывестиИнформациюОПамятиПроцессов1С();
			Исключение
