...
Функция StepDefinitionИзТаблицыСценариев(IDИзStepDefinition,ТаблицаУжеСуществующихСценариев,СтруктураПараметров,НовыйStepDefinition)
	...
	Если СтрТаблицаУжеСуществующихСценариев = Неопределено и ТекущийЯзыкФичаФайла <> "ru" И ЗначениеЗаполнено(ТекущийЯзыкФичаФайла)  Тогда
...
Функция StepDefinitionИзТаблицы(IDИзStepDefinition,ТаблицаИзвестныхStepDefinition,СтруктураПараметров,ЗначенияПараметров,СтрокаДереваШаг)
	...
	Иначе	
		...
		Если СтрокаТаблицаПеревода = Неопределено Тогда
			...
			Если СтрокаТаблицаПеревода <> Неопределено Тогда
				Поз = Найти(СтрокаТаблицаПеревода.ТекстПереводаШаг, Символы.ПС);
				Если Поз > 0 Тогда
					СтрокаДереваШаг.Имя = СокрП(Лев(СтрокаТаблицаПеревода.ТекстПереводаШаг, Поз - 1));
				Иначе	
...
Процедура ЗакрытьПредыдущийСценарий(ИмяФичи,ПолноеИмяФичи,ТекущийТипСценария,ПредыдущееКлючевоеСлово,ТекущееКлючевоеСлово,ДеревоСтроки,ОписаниеСценария,ТаблицаШагов,ТаблицаИзвестныхStepDefinition,ТаблицаСтрокПримеров,НомерСтрокиСценария,ТаблицаУжеСуществующихСценариев,СтруктураПараметров,МассивСценариевЗащитаОтЗацикливанияКеш,ЭтоЗагрузкаПодчиненногоСценария,ПередаваемыеТаблицы,ИмяЗагружаемогоСценария)
	...
	Если СтруктураПараметров.Свойство("ОтборСценариевПоИмениСценария") Тогда
		Если СтруктураПараметров.ОтборСценариевПоИмениСценария[ИмяСценария] = Неопределено Тогда
			Возврат;
		КонецЕсли;	
	КонецЕсли;	
	...
	Если (ТекущийТипСценария = "background") или (ТекущийТипСценария = "scenario") или (ТекущийТипСценария = "scenario_outline") Тогда
		...
		Для СчетчикТаблицыШагов = 0 По ТаблицаШагов.Количество()-1 Цикл
			...
			Если НЕ ПустаяСтрока(СтрТаблицаШагов.ИмяОбласти) ИЛИ СтрТаблицаШагов.КлючевоеСлово = "if" ИЛИ СтрТаблицаШагов.КлючевоеСлово = "elseif" ИЛИ СтрТаблицаШагов.КлючевоеСлово = "else" Тогда
				
				Если СтрТаблицаШагов.НачалоОбласти ИЛИ СтрТаблицаШагов.КлючевоеСлово = "if" ИЛИ СтрТаблицаШагов.КлючевоеСлово = "elseif" ИЛИ СтрТаблицаШагов.КлючевоеСлово = "else" Тогда
					...
					Если Лев(СтрТаблицаШагов.ИмяОбласти,1) <> "*" Тогда
						...
						Если СтрТаблицаИзвестныхStepDefinition = Неопределено И ТекущийЯзыкФичаФайла <> "ru" И ЗначениеЗаполнено(ТекущийЯзыкФичаФайла) Тогда
							ТаблицаПеревода = СтруктураПараметров.ТаблицаПеревода;
							СтрокаТаблицаПеревода = ТаблицаПеревода.Найти(IDИзStepDefinition,"СтрокаДляПоискаПеревод");
			...
			Иначе
				...
				Иначе
					...
					Если УниверсальноеИмяФайла(ПолноеИмяФичи) = УниверсальноеИмяФайла(СтрТаблицаУжеСуществующихСценариев.ИмяФайла)
						ИЛИ СтрТаблицаУжеСуществующихСценариев.ДвоичныеДанные <> Неопределено Тогда
						СтрКешФич = Неопределено;
					Иначе	
					...
					Если СтрКешФич = Неопределено Тогда
						...
						Иначе
							...
							Если НЕ СтруктураПараметров.Свойство("КешВременныхФичаФайлов") Тогда
								СтруктураПараметров.Вставить("КешВременныхФичаФайлов", Новый Соответствие);
							КонецЕсли;
							
							УниверсальноеИмя = УниверсальноеИмяФайла(СтрТаблицаУжеСуществующихСценариев.ИмяФайла);
							ЗначениеИзКеш = СтруктураПараметров.КешВременныхФичаФайлов[УниверсальноеИмя];
							Если ЗначениеИзКеш = Неопределено Тогда
								...
								СтруктураПараметров.КешВременныхФичаФайлов.Вставить(УниверсальноеИмя, ИмяВременнойФичи);
							Иначе	
								ИмяВременнойФичи = ЗначениеИзКеш;
							КонецЕсли;	
						...
						Если НЕ ВФичеЕстьСтруктурыСценария Тогда
							Если УниверсальноеИмяФайла(ПолноеИмяФичи) <> УниверсальноеИмяФайла(СтрТаблицаУжеСуществующихСценариев.ИмяФайла) Тогда
			...
			Если ПараметрыКопирования.Свойство("ЗначенияПараметровДляЗаменыВРодительскомСценарии") Тогда
				Если ПараметрыКопирования.ЗначенияПараметровДляЗаменыВРодительскомСценарии.Количество() > 0 Тогда
					...
					Для каждого РодительскийПараметр Из ПараметрыКопирования.ЗначенияПараметровДляЗаменыВРодительскомСценарии Цикл
						...
						СтрокаДляЗаментыПараметров = ЗаменаПараметровВСтроке(СтрокаДляЗаментыПараметров, ЧтоЗаменяем, НаЧтоЗаменяем, ВложенныйПараметр.Тип, РодительскийПараметр.Тип, Ложь);
					...
					ПолучитьКлючевоеСлово(НРег(СтрокаКуда.Имя), ВтораяЧастьСтрокиПозиция);
					...
					Если СтрокаКуда.ЗначенияПараметров <> Неопределено Тогда
						Для Каждого ТекЗн Из СтрокаКуда.ЗначенияПараметров Цикл
							Если ТекЗн.Значение.Свойство("Многострочный") Тогда
								...
								Ном = -1;
								СтрокаДляЗаментыПараметров = ТекЗн.Значение.Значение;
								Для каждого РодительскийПараметр Из ПараметрыКопирования.ЗначенияПараметровДляЗаменыВРодительскомСценарии Цикл
									Ном = Ном + 1;
									ВложенныйПараметр = ПараметрыКопирования.ЗначенияПараметровДляЗаменыВоВложенномСценарии[Ном];
									Если ТипЗнч(ВложенныйПараметр.Значение) = Тип("Структура") Тогда
									...
									СтрокаДляЗаментыПараметров = ЗаменаПараметровВСтроке(СтрокаДляЗаментыПараметров, ЧтоЗаменяем, НаЧтоЗаменяем, "Строка", РодительскийПараметр.Тип, Истина);
								...
								МногострочныеПараметрыШага.Добавить(СтрокаДляЗаментыПараметров);
							КонецЕсли;	 
						КонецЦикла;	 
					КонецЕсли;	 
					...
					ЗначенияПараметров = ПараметрыДляПолученияStepDefenition.ЗначенияПараметров;
					IDИзStepDefinition = НРег(Лев(НовыйStepDefinition,Найти(НовыйStepDefinition,"(")-1));
					StepDefinitionИзТаблицы(IDИзStepDefinition,СтруктураПараметров.ТаблицаИзвестныхStepDefinition,СтруктураПараметров,ЗначенияПараметров,СтрокаКуда);
...
Функция ЗаменаПараметровВСтроке(Знач СтрокаДляЗаментыПараметров, ЧтоЗаменяем, НаЧтоЗаменяем, ТипПараметра, РодительскийПараметрТип, ТолькоКвадратныеСкобки)
	МассивЧтоЗаменяем1 = Новый Массив;
	МассивЧтоЗаменяем2 = Новый Массив;
	
	Если ТолькоКвадратныеСкобки Тогда
		Если ТипПараметра = "Строка" Тогда
	...
	Если РодительскийПараметрТип = "Строка" Тогда
		НаЧтоЗаменяем = "'" + НаЧтоЗаменяем + "'";
	ИначеЕсли РодительскийПараметрТип = "ПараметрИменованный" Тогда
		НаЧтоЗаменяем = "<" + НаЧтоЗаменяем + ">";
	КонецЕсли;	 
	...
	Возврат СтрокаДляЗаментыПараметров;
...
Функция ПроверитьЧтоМожноСоздатьОбласть(ТаблицаТекстФичи,СчетчикСтрокФичи, ЗначениеОтступа)
	...
	Пока Истина Цикл
		...
		Если Лев(СокрЛ(СтрТаблицаТекстФичи.Стр), 3) = """""""" Тогда
			Возврат Ложь;
		КонецЕсли;	
...
Функция ПроверитьФичуНаКорректностьСинтаксисаИЗагрузитьСценарии(Знач ИмяФайла,ОписаниеОшибки,ДеревоСтроки,ТаблицаИзвестныхStepDefinition,СтруктураПараметров,МассивСценариевЗащитаОтЗацикливанияКеш,ЭтоЗагрузкаПодчиненногоСценария,ПередаваемыеТаблицы,ИмяЗагружаемогоСценария)
	...
	Для СчетчикСтрокФичи = 1 По КолСтрокТаблицаТекстФичи Цикл
		...
		Иначе	
			Если Лев(Стр,1) = "|" И НЕ ИдетСчитываниеМногострочногоПараметра Тогда
				...
				Если ОблатьЗакрывается(НадоСтроитьДерево, НачалсяСценарий, ТекущееКлючевоеСлово, ТаблицаТекстФичи, СчетчикСтрокФичи, МассивТеговШага, МассивКомментариевШага, ТаблицаШагов, СтрокаТаблицаТекстФичи, ИдетСчитываниеМногострочногоПараметра) Тогда
					Продолжить;
				КонецЕсли;
...
Функция ОблатьЗакрывается(НадоСтроитьДерево, НачалсяСценарий, ТекущееКлючевоеСлово, ТаблицаТекстФичи, СчетчикСтрокФичи, МассивТеговШага, МассивКомментариевШага, ТаблицаШагов, СтрокаТаблицаТекстФичи, ИдетСчитываниеМногострочногоПараметра)
...
Функция ВставитьВПараметрыСимволыПроцента(Знач Стр, СтруктураПараметров)
	...
	ПолучитьStepDefinitionПоСтроке(ПерваяЧастьСтроки,ЗначенияПараметров,,,,ДанныеОбработкиПараметров,СтруктураПараметров);
...
Функция ЕстьЗначащиеСимволыStepDefinition(Знач Стр)
...
Процедура ПрочитатьФайлПеревода(ДанныеПеревода)
	...
	Для Ккк = 2 По ТабДок.ВысотаТаблицы Цикл
		...
		Если СтрокаТаблицаПеревода = Неопределено Тогда
			Если ЕстьЗначащиеСимволыStepDefinition(ОписаниеШагаРусский.StepDefinition) Тогда
...
Процедура ПолучитьСнипетыДляТаблицыПеревода(ДанныеПеревода)
	...
	Для Каждого СтрокаТаблицаПеревода Из ТаблицаПеревода Цикл
		...
		КоличествоПараметровРусский = ОписаниеШагаРусский.ЗначенияПараметров.Количество();
		...
		КоличествоПараметровПеревод = ОписаниеШагаПеревод.ЗначенияПараметров.Количество();
		Если КоличествоПараметровРусский <> КоличествоПараметровПеревод И ЗначениеЗаполнено(ОписаниеШагаПеревод.ТекстПереводаШаг) Тогда
			Стр = "Для шага <%1> не совпало количество параметров. Было <%2>, стало <%3>.";
			Стр = СтрЗаменить(Стр, "%1", СтрокаТаблицаПеревода.ОригиналРусскийШаг);
			Стр = СтрЗаменить(Стр, "%2", КоличествоПараметровРусский);
			Стр = СтрЗаменить(Стр, "%3", КоличествоПараметровПеревод);
			Ошибки.Добавить(Стр);
		КонецЕсли;	
	...
	Если Ошибки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаОшибки Из Ошибки Цикл
			Стр = Стр + СтрокаОшибки + Символы.ПС;
		КонецЦикла;
...
Функция ВернутьВСтрокуПараметры(Знач Стр,ДанныеПеревода,ДанныеОбработкиПараметровРусский,ДанныеОбработкиПараметровПеревод, МассивСоответствийПозицийПараметров)
	КолПараметров        = 0;
	КолПараметровЧисло   = 0;
	КолПараметровСтрокаА = 0;
	КолПараметровСтрокаК = 0;
	...
	Для Каждого Элем Из МассивПодстрок Цикл
		...
		Если Лев(МассивСтрокЗначениеПараметраПеревод[0],1) = "%" Тогда
			...
			Если МассивСоответствийПозицийПараметров <> Неопределено И МассивСоответствийПозицийПараметров.Количество() > 1 Тогда
				НомерПараметра = МассивСоответствийПозицийПараметров[НомерПараметра-1];
			КонецЕсли;	
...
Функция ПеревестиТекстПоТаблицеПеревода(ДанныеПеревода)
	...
	Если ДанныеПеревода.Свойство("ИсходныйЯзык") Тогда
		ТекущийЯзыкФичаФайла  = ДанныеПеревода.ИсходныйЯзык;
		ДанныеИсходногоЯзыка = ПолучитьИзВременногоХранилища(ДанныеПеревода.КешДанныеПеревода.СоответствиеПоЯзыкам[ТекущийЯзыкФичаФайла]);
	Иначе
		ТекущийЯзыкФичаФайла  = "ru";
		ДанныеИсходногоЯзыка = Неопределено;
	КонецЕсли;
	
	ТаблицаКлючевыхСловОригинал = ДанныеПеревода.КлючевыеСловаОригинал.ТаблицаКлючевыхСлов;
	...
	Для Ккк = 0 По МассивСтрок.Количество()-1 Цикл
		...
		Если ОписаниеШагаРусский.КлючевоеСлово = "feature" ИЛИ ОписаниеШагаРусский.КлючевоеСлово = "scenario" Тогда
			СтрокаТаблицаКлючевыхСловПеревод = ТаблицаКлючевыхСловПеревод.Найти(ОписаниеШагаРусский.КлючевоеСлово, "Тип");
			Если СтрокаТаблицаКлючевыхСловПеревод <> Неопределено Тогда
				НовоеСлово = СделатьПервуюБуквуЗаглавной(СтрокаТаблицаКлючевыхСловПеревод.Слово);
				МассивСтрокПеревод.Добавить(НовоеСлово + ОписаниеШагаРусский.ДанныеОбработкиПараметров.ОбработаннаяСтрокаПараметров);
				Продолжить;
			КонецЕсли;	
		КонецЕсли;	
		...
		// Обработка случая, когда идёт перевод с другого языка на русский
		Если ТекущийЯзыкФичаФайла <> "ru" И ЗначениеЗаполнено(ТекущийЯзыкФичаФайла) И ДанныеПеревода.ЯзыкПеревода = "ru" Тогда
			СтрокаТаблицаПеревода = ДанныеИсходногоЯзыка.Найти(StepDefinitionРусский, "StepDefinitionПеревод");
			Если СтрокаТаблицаПеревода <> Неопределено и СтрокаТаблицаПеревода.StepDefinitionРусский <> "_()" Тогда
				Если СтрокаТаблицаПеревода.ОригиналРусскийОписание = "Специальный текст" Тогда
					МассивСтрокПеревод.Добавить(СтрокаТаблицаПеревода.ТекстПереводаШаг);
					Продолжить;
				...
				Попытка
					СтрокаСПравильнымиПараметрами = ВернутьВСтрокуПараметры(
					СтрокаТаблицаПеревода.ОбработаннаяСтрокаПараметровРусский,ДанныеПеревода,
					ОписаниеШагаРусский.ДанныеОбработкиПараметров,СтрокаТаблицаПеревода.ДанныеОбработкиПараметровРусский,
					СтрокаТаблицаПеревода.МассивСоответствийПозицийПараметров);
				Исключение
					ТекстСообщения = ПолучитьТекстСообщенияПользователю("Не получилось подставить в строку параметры: <%1>");
					ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",СтрокаТаблицаПеревода.ОбработаннаяСтрокаПараметровРусский);    
					ТекстСообщения = ТекстСообщения + " " + ОписаниеОшибки();
...
Функция СделатьПереводТекстаGherkin(ДанныеПеревода) Экспорт
	...
	ИсходныйЯзык = "ru";
	Если ДанныеПеревода.Свойство("ИсходныйЯзык") Тогда
		ИсходныйЯзык = ДанныеПеревода.ИсходныйЯзык;
	КонецЕсли;	
	
	ДанныеПеревода.Вставить("КлючевыеСловаОригинал",СоответствиеТаблицПереводов[ИсходныйЯзык]);
	...
	ДанныеПеревода.Вставить("КлючевыеСловаОригинал",Неопределено);
...
Функция ПолучитьПереводТекстаGherkin(ДанныеПеревода) Экспорт
	...
	Для Каждого СтрокаТаблицаДляПереводаИзвестныхШагов Из ДанныеПеревода.ТаблицаДляПереводаИзвестныхШагов Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицаДляПереводаИзвестныхШагов.ОригиналРусскийШаг) Тогда
			Продолжить;
		КонецЕсли;	
	...
	ДанныеПеревода.Вставить("КлючевыеСловаОригинал",СоответствиеТаблицПереводов["ru"]);
	ДанныеПеревода.Вставить("КлючевыеСловаПеревод",СоответствиеТаблицПереводов[ДанныеПеревода.ЯзыкПеревода]);
