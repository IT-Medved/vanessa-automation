...
Процедура ОбработчикПодключенияПриОткрытии(Знач ПомещенныеФайлы, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		ИмяФайла = ДополнительныеПараметры.ИменаФайлов["ИмяФайла"];
		ДополнительныеПараметрыПодключения.Вставить("ИмяФайла",ИмяФайла);
		...
		ПодключениеОбработкиПродолжение(ИмяОбработки, ИмяФайла);
	КонецЦикла;	 
КонецПроцедуры
...
&НаКлиенте
Процедура ПодключениеОбработкиПродолжение(ИмяОбработки, ИмяФайла)
	
	ПараметрыОбработки = Новый Структура;
	
	ЭтоОбработка = Истина;
	Если Прав(СокрЛП(НРег(ИмяФайла)),4) = ".erf" Тогда
		ЭтоОбработка = Ложь;
	КонецЕсли;
	...
	Иначе	
		Если ЗначениеЗаполнено(ЗначениеИмяФормы) Тогда
			ОткрытьФорму("ВнешнийОтчет." + ИмяОбработки + ".Форма." + ЗначениеИмяФормы, ПараметрыОбработки);
		Иначе	
