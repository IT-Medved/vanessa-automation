...
Функция ПолучитьМакетСервер()
	...
	ИначеЕсли ИмяОтчета = "ОстаткиИДоступностьТоваров" Тогда
		
		НастройкиФормы = Вычислить("ВариантыОтчетов").НастройкиФормыОтчета(ИдентификаторОбъектовМетаданных, КлючВарианта, Отчет);
		Если НастройкиФормы.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
			Отчет.ПередЗагрузкойНастроекВКомпоновщик(
				Результат,
				"",
				КлючВарианта,
