...
Сценарий: Передача параметров во вложенный сценарий
	...
	Теперь напишем простой feature файл по созданию элемента справочника
			И видеовставка 'В принципе его можно накликать мышкой, но мы посмотрим как это делается кодом'
			...
			И видеовставка 'видеовставка на параметр "Элемент". Вызывая сценарий извне, мы будем менять его значение.'
			...
			И видеовставка 'видеовставка на тег ExportScenarios. Его нужно указать, чтобы сценарий можно было вызывать из других feature файлов.'
	...
	Теперь рядом создадим второй feature файл, который будет вызывать только что созданный сценарий
			...
			И видеовставка 'Теперь мы будем вызывать подчинённый сценарий 3 раза с разными параметрами'
	...
	Теперь проверим работу второго feature файла
			...
			И видеовставка 'Таким образом было создано три элемента справочника Товары'
