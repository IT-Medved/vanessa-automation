...
Сценарий: Работа с таймером
	...
	Далее мы напишем шаги, которые создают и проводят документ. И мы заранее не знаем сколько будет проводиться наш документ.
			...
			И видеовставка 'Этот шаг сохранит в переменную Контекст значение поля Номер, т.е. в коде можно будет писать: Контекст.НомерДокумента'
			И я пишу 'И я нажимаю на кнопку "Провести и закрыть" '
			И я пишу 'И я ожидаю окончания проведения документа1 '	
			И видеовставка 'В этом шаге мы будем использовать таймер, т.к. мы предполагаем, что документ может проводиться достаточно долго. '
	...
	Далее надо написать код в полученных процедурах
			...
			И видеовставка 'видеовставка. В модуле была создана только одна процедура, хотя шагов в фиче было больше.'
			И видеовставка 'Это происходит благодаря механизму повторного использования шагов.'
			...
			И видеовставка 'Теперь мы остановили выполнение шагов, пока не выполнится нужное нам условие.'
