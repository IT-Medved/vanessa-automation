...
#	И в открытой форме я открываю выпадающий список с заголовком "Реквизит1"
	...
	Иначе
		...
		И я нажимаю кнопку выбора у поля "Реквизит список"
		И я сохраняю выпадающий список "Реквизит список" как "ДанныеВыпадающегоСписка"
		И для каждого значения "ЗначениеИзМассива" из массива в памяти "ДанныеВыпадающегоСписка"
			И я вывожу значение переменной "ОтображаемыйТекст"

		//Проверяем последнее значение из выпадающего списка. Это "Тест"
		И переменная "ОтображаемыйТекст" имеет значение "Тест" Тогда

		Если версия платформы ">=" "8.3.25" Тогда
			И я сохраняю выпадающий список формы как "СписокФормы"
			И выражение внутреннего языка 'Контекст.СписокФормы.Количество() = 2' Истинно
