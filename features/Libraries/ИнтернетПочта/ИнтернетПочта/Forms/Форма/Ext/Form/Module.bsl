...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"Проверяет, что в ящике есть новые сообщения, и сохраняет их в переменную Контекст. Отбор использует флаг Recent, который сбрасывается любым обращением к ящику, поэтому проверка может быть нестабильной.",
		"Плагины.ИнтернетПочта");
...
Процедура ЯОтвечаюНаЭтоСообщение() Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторСообщения) Тогда
		Если Не СтрНачинаетсяС(ИдентификаторСообщения, "<") Тогда
