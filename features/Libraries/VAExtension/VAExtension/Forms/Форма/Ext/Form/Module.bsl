...
&НаКлиенте
Функция ЗаголовокОкнаПроверкаФоновыхЗаданий()
	Возврат "VAExtension проверка фоновых заданий";
КонецФункции
...
Функция ЯЖдуЗавершенияВыполненияВсехФоновыхЗаданийРасширениеОжиданиеОбновлениеТаблицы()
	...
	Попытка
		ОкноРасширения = КонтекстСохраняемый.ТестовоеПриложение.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"), ЗаголовокОкнаПроверкаФоновыхЗаданий());
		...
		Иначе
			...
			Если АктивноеОкно.Заголовок <> ЗаголовокОкнаПроверкаФоновыхЗаданий() Тогда
		...
		Попытка
			...
			Если ЗначениеТаблицы <> Неопределено Тогда
				Если ЗначениеТаблицы.Количество() = 0 Тогда
					Ванесса.ЯЗакрываюОкноСЗаголовком("*Проверка*фоновых*заданий");
