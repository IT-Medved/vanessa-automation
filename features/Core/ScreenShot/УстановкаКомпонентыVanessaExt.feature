		...
		Попытка
			Если 'Объект.ВерсияПоставки = "standart"' тогда
				Когда Я подключаю клиент тестирования с параметрами:
					...
					| 'ПодключениеКомпоненты' | '$ИмяПользователя$' | '$КаталогИнструментов$/vanessa-automation.epf' | '/C "NoFeatureLoadOnOpenForm;DisableFirstRunHelper;UseEditor=False"' |
			Иначе			
				Когда Я подключаю клиент тестирования с параметрами:
					| 'Имя'                   | 'Логин'             | 'Запускаемая обработка'                               | 'Дополнительные параметры строки запуска'                             |
					| 'ПодключениеКомпоненты' | '$ИмяПользователя$' | '$КаталогИнструментов$/vanessa-automation-single.epf' | '/C "NoFeatureLoadOnOpenForm;DisableFirstRunHelper;UseEditor=False" ' |
