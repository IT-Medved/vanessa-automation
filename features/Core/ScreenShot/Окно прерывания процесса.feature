...
Сценарий: Показать окно прерывания процесса
	...
	* Показать окно прерывания
		...
		И Я запоминаю значение выражения 'ВнешняяКомпонентаДляСкриншотов.ПолучитьСписокОкон()' в переменную "СостояниеОконДо"
	...
	* Проверить существование окна прерывания и закрытие его кликом мышки
		...
		И я делаю 3 раз
			...
			Для каждого Стр из ПрочитатьОбъектJSON(ТекстJSON) Цикл
				Если Стр.Class = "VanessaTerminator" Тогда
					...
					Сообщить("РазмерыОкна.Right=" + РазмерыОкна.Right);
					Сообщить("РазмерыОкна.Top=" + РазмерыОкна.Top);
		...
		И Я запоминаю значение выражения 'ВнешняяКомпонентаДляСкриншотов.ПолучитьСписокОкон()' в переменную "СостояниеОконПосле"
		И Пауза 1
