...
@IgnoreOn8315
@IgnoreOn8316
Сценарий: Проверка получения состояния отображения табличного документа
	...
	И состояние табличного документа "РеквизитТабличныйДокумент" равно ''
	И я жду состояния табличного документа "РеквизитТабличныйДокумент" '' в течение 10 секунд
	И я запоминаю состояние табличного документа "РеквизитТабличныйДокумент" в переменную "ИмяПеременной"
	И переменная "ИмяПеременной" имеет значение "" 
...
//https://github.com/Pr-Mex/vanessa-automation/issues/2403
Сценарий: Проверка особого случая сравнения макетов с переносом строки
	И я закрываю все окна клиентского приложения
	
	И я открываю табличный документ "$КаталогФичи$\files\МакетССлешН.mxl" (расширение)
	Тогда табличный документ "SpreadsheetDocument" содержит строки из макета "МакетССимволомПС"
...
Сценарий: Проверка значения области с пропусками
	...
	И я выбираю файл "$КаталогИнструментов$\features\Core\ExpectedSomething\files\МакетСПропусками.mxl"
	И я нажимаю на кнопку с именем 'ФормаОткрытьМакет'
	И Пауза 1
	
	И я запоминаю значение настройки Vanessa Automation "ПолучатьМакетыСПомощьюРасширенияVAExtension" в "ПолучатьМакетыСПомощьюРасширенияVAExtension"
	
	И я снимаю флаг настройки Vanessa Automation "ПолучатьМакетыСПомощьюРасширенияVAExtension"
	И область "R3C1:R7C3" табличного документа "РеквизитТабличныйДокумент" равна макету "МакетСПропускамиЭталонОбласть"
	И область "R3C1:R7C3" табличного документа "РеквизитТабличныйДокумент" равна макету "МакетСПропускамиЭталонОбласть" по шаблону
		
	И я устанавливаю флаг настройки Vanessa Automation "ПолучатьМакетыСПомощьюРасширенияVAExtension"
	И область "R3C1:R7C3" табличного документа "РеквизитТабличныйДокумент" равна макету "МакетСПропускамиЭталонОбласть"
	И область "R3C1:R7C3" табличного документа "РеквизитТабличныйДокумент" равна макету "МакетСПропускамиЭталонОбласть" по шаблону

	// Если изначально опция была выключена, то выключаем её
	Если 'НРег(Строка($ПолучатьМакетыСПомощьюРасширенияVAExtension$)) = "нет"' Тогда
		И я снимаю флаг настройки Vanessa Automation "ПолучатьМакетыСПомощьюРасширенияVAExtension"
...
Сценарий: Проверка редактирования табличного документа и сохраннения его в файл
...
Сценарий: Сохранение табличного документа в файл
	...
	И В открытой форме я нажимаю на кнопку с заголовком "Записать"
	И я нажимаю на кнопку с именем 'СформироватьОтчет'
	Тогда я удаляю файл "c:\temp\Testmxl.mxl"
	И я выбираю файл "c:\temp\Testmxl.mxl"
	И я сохраняю данные элемента формы с именем "РеквизитТабличныйДокумент" в файл
	И Пауза 2
	И файл "c:\temp\Testmxl.mxl" существует
...
Сценарий: Проверка получения макета когда данные получены два раза в одном окне
	...
	Если версия платформы ">=" "8.3.13" Тогда
		И я жду, что табличный документ "РеквизитТабличныйДокумент" заполнится данными в течение 10 секунд
...
Сценарий: Проверка макета, когда используются значения контекста
	...
	И я удаляю переменную 'глБукваЕ'
	И я запоминаю значение выражения '"е"' в переменную "глБукваЕ" глобально
	И я запоминаю значение выражения '"с"' в переменную "БукваС"
	Дано Табличный документ "РеквизитТабличныйДокумент" равен макету "МакетСПеременнымиКонтекста6"

	И я запоминаю значение выражения '"е"' в переменную "БукваЕ"
	И я удаляю переменную 'глБукваС'
	И я запоминаю значение выражения '"с"' в переменную "глБукваС" глобально
	Дано Табличный документ "РеквизитТабличныйДокумент" равен макету "МакетСПеременнымиКонтекста5"

	И я удаляю переменную 'глБукваЕ'
	И я удаляю переменную 'глБукваС'
	И я запоминаю значение выражения '"е"' в переменную "глБукваЕ" глобально
	И я запоминаю значение выражения '"с"' в переменную "глБукваС" глобально
	Дано Табличный документ "РеквизитТабличныйДокумент" равен макету "МакетСПеременнымиКонтекста4"
...
Сценарий: Я запоминаю значение макета и потом сравниваю с текущим макетом
	...
	Если версия платформы ">=" "8.3.25" Тогда
		И в табличном документе 'РеквизитТабличныйДокумент' в ячейке 'R5C2' текст находится в пределах границ области
		И в табличном документе 'РеквизитТабличныйДокумент' область 'R5C2' не входит в объединенную область
...
Сценарий: Я получаю данные макета где есть объединенные ячейки
	...
	И в табличном документе 'РеквизитТабличныйДокумент' я перехожу к ячейке "R1C1:R2C2"
	И в табличном документе 'РеквизитТабличныйДокумент' я перехожу на '2' ячеек вправо
...
Сценарий: Я получаю данные макета открытого отдельным окном
	...
	И я запоминаю строку "Тест" в переменную "ПеремТест"
	И табличный документ "РеквизитТабличныйДокумент" содержит строки из макета "ДанныеМакетаОтдельноеОкно"
	И табличный документ "РеквизитТабличныйДокумент" содержит строки из макета "$КаталогИнструментов$\features\Core\ExpectedSomething\files\ДанныеМакетаОтдельноеОкно.mxl"
...
Сценарий: Проверка перехода к ячейке макета
	Когда я удаляю все элементы Справочника "Справочник3"
	...
	И в открытой форме я нажимаю на кнопку с заголовком "Создать"
	Тогда открылось окно "Справочник3 (создание)"
	И в открытой форме в поле с заголовком "Наименование" я ввожу текст "Тест1"
	Когда открылось окно 'Справочник3 (создание)*'
	И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
	И я жду закрытия окна 'Справочник3 (создание) *' в течение 20 секунд
	Тогда открылось окно 'Справочник3'
	И я нажимаю на кнопку с именем 'ФормаСоздать'
	Тогда открылось окно 'Справочник3 (создание)'
	...
	Тогда открылось окно 'Тест2 (Справочник3)'
	И я нажимаю на кнопку с именем 'СформироватьОтчет'
	И Пауза 1
	И в табличном документе "РеквизитТабличныйДокумент" я перехожу к ячейке содержащей текст 'Тест*' по шаблону
	И я запоминаю значение текущей ячейки "РеквизитТабличныйДокумент" в переменную "ИмяПеременной"
	И переменная "ИмяПеременной" имеет значение "Тест1"
