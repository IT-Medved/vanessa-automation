...
Сценарий: Я запускаю клиент тестирования с эмуляцией запуска из командной строки "ИмяJson" "ДопПараметрКоманднойСтроки" "ОставитьЗагрузчикНастроек" "УбратьПараметрПоказаОсновнойФормы" "УбратьПараметрStartFeaturePlayer"
	И я запоминаю строку "ИмяJson" в переменную "ИмяПеременнойJson"
	И я запоминаю строку "УбратьПараметрStartFeaturePlayer" в переменную "ИмяПеременнойУбратьПараметрStartFeaturePlayer"
	...
	И я запоминаю строку "УбратьПараметрПоказаОсновнойФормы" в переменную "$ПеременнаяУбратьПараметрПоказаОсновнойФормы$"
	Если '$ПеременнаяУбратьПараметрПоказаОсновнойФормы$ = "УбратьПараметрПоказаОсновнойФормы"' Тогда
		И я удаляю переменную 'ПеременнаяУбратьПараметрПоказаОсновнойФормы'
		И я запоминаю строку "" в переменную "$ПеременнаяУбратьПараметрПоказаОсновнойФормы$"
		

	Если 'ЗначениеЗаполнено(Контекст.ИмяПеременнойJson)' Тогда
	...
	И Я запоминаю значение выражения '" /TESTCLIENT -TPort 48111 /Execute " + Ванесса.Объект.КаталогИнструментов + "/" + $ИмяEpf$ + " /CStartFeaturePlayer;ShowMainForm;DisableUserSettingsLoader;NoLoadConfig;"' в переменную "ДопПараметр"
	Если 'ЗначениеЗаполнено(Контекст.ИмяПеременнойJson)' Тогда
		И Я запоминаю значение выражения 'Контекст.ДопПараметр + "VBParams=" + $TestJson$ + ";"' в переменную "ДопПараметр"

	Если '$ПеременнаяУбратьПараметрПоказаОсновнойФормы$ = "Истина"' Тогда
		И Я запоминаю значение выражения 'СтрЗаменить($ДопПараметр$,"ShowMainForm;","")' в переменную "ДопПараметр"
	...
	Если переменная "ИмяПеременнойУбратьПараметрStartFeaturePlayer" имеет значение "Да" Тогда
		И Я запоминаю значение выражения 'СтрЗаменить(Контекст.ДопПараметр, "StartFeaturePlayer;", "")' в переменную "ДопПараметр"
