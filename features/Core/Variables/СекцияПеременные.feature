...
//https://github.com/Pr-Mex/vanessa-automation/issues/1596
Сценарий: Проверка цикла по таблице из секции переменных. Старый парсер.
	...
	* Выключение использования компоненты	
		И я снимаю флаг с именем 'ИспользоватьКомпонентуVanessaExt'
				
	
	* Загрузка служебной фичи
		Когда В поле с именем "КаталогФичСлужебный" я указываю путь к служебной фиче "СекцияПеременные/СекцияПеременные06"
	...
	* Проверка результата
		Тогда в логе сообщений TestClient есть строки:
			|'Имя = ГлавныйКонтрагент2'|
			|'Представление = ООО Моя Фирма2'|
			|'Выражение = 4'|
			|'Имя = ГлавныйКонтрагент3'|
			|'Представление = ООО Моя Фирма3'|
			|'Выражение = 5'|
								

//https://github.com/Pr-Mex/vanessa-automation/issues/1596
Сценарий: Проверка цикла по таблице из секции переменных. Новый парсер.
	* Проверка, что выключен новый парсер
		И я перехожу к закладке с именем "ГруппаНастройки"
		И элемент формы с именем "ИспользоватьПарсерGherkinИзКомпонентыVanessaExt" стал равен 'Нет'
	...
	* Включение использования нового парсера
...
Сценарий: Проверка фичи, где в секции переменные есть только таблица. Старый парсер.
	* Проверка, что выключен новый парсер
		И я перехожу к закладке с именем "ГруппаНастройки"
		И элемент формы с именем "ИспользоватьПарсерGherkinИзКомпонентыVanessaExt" стал равен 'Нет'
	...
	* Загрузка служебной фичи
		Когда В поле с именем "КаталогФичСлужебный" я указываю путь к служебной фиче "СекцияПеременные/СекцияПеременные05"
...
Сценарий: Инициализация простых переменных из секции переменных. Старый парсер.
	...
	* Проверка результата
		Тогда в логе сообщений TestClient есть строки:
			...
			| 'Перем14 = Июнь 2022 г.'                            |
			| 'Перем15 = 30.12.2022 0:00:00'                      |
			| 'Перем16 = 01.01.2022 0:00:00'                      |
			| 'Перем17 = 01.01.2022 0:00:00'                      |
			| 'ИмяПеременной = aaa{bbb}ccc'                       |
			| 'ПроверкаУсловия = УловиеНашлоПеременную'           |
...
Сценарий: Передача простых переменных из секции переменных в шаги. Старый парсер.
	...
	* Проверка результата
		Тогда в логе сообщений TestClient есть строки:
			...
			| 'ИмяПеременной14 = 4'                                                               |
			| 'ИмяПеременной16 = Июнь 2022 г.'                                                    |
		
		// Обход ошибки в старых версиях платформы
		Если версия платформы ">=" "8.3.18.1" Тогда
			Тогда в логе сообщений TestClient есть строки:
				| 'Тип переменной <ИмяПеременной15> = Дата'                                           |
