...
Сценарий: Проверка существования файла
	...
	//Проверка https://github.com/Pr-Mex/vanessa-automation/issues/2231
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
	"""
		Сообщить(УбратьПарольПользователяИзСтроки("/N111 /p""234"" /ДопПараметр"));
	"""
	...
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
	"""
		Сообщить(УбратьПарольПользователяИзСтроки("/N111 /P""234"""));
	"""
	И я очищаю окно сообщений пользователю
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 3
	...
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
	"""
		Сообщить(УбратьПарольПользователяИзСтроки("/N111 /P234"));
	"""
	И я очищаю окно сообщений пользователю
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 3
	...
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
	"""
		Сообщить(УбратьПарольПользователяИзСтроки("/N111 /P234 /ДопПараметр"));
	"""
	И я очищаю окно сообщений пользователю
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 3
	...
	//Проверка https://github.com/Pr-Mex/vanessa-automation/issues/1945
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
	"""
		Значение = Новый Массив;
		ЗначениеМассива = Новый Соответствие;
		ЗначениеМассива.Вставить("111" + Символы.ПС + "222", "Значение1");
		Значение.Добавить(ЗначениеМассива);
		МассивКлючей = Новый Массив;
		МассивКлючей.Добавить("111\n222");
		Макет = ПреобразоватьЗначениеВМакет(Значение, МассивКлючей);
		Сообщить(Макет.Область(2,1,2,1).Текст);
	"""
	И я очищаю окно сообщений пользователю
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 3
...
Сценарий: Вызов некоторых процедур напрямую. Вычисление даты.
	...
	И я очищаю окно сообщений пользователю
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
		|'Сообщить(УбратьТехническиеДанныеИзСтроки("{ВнешняяОбработка.РаботаСИнтерфейсомДополнительный.Форма.Форма.Форма(386)}:ЧастьВнутри1{ВнешняяОбработка.РаботаСИнтерфейсомДополнительный.Форма.Форма.Форма(368)}:ЧастьВнутри2{ВнешняяОбработка.VanessaAutomation.МодульОбъекта(4701)}: [Failed] Таблицы должны совпадать, а они различны."))'|
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 1
	Тогда не появилось окно предупреждения системы
	Тогда в логе сообщений TestClient есть строки
		|'ЧастьВнутри1ЧастьВнутри2[Failed] Таблицы должны совпадать, а они различны.'|
		
		
	Тогда в логе сообщений TestClient нет строки "{"
	Тогда в логе сообщений TestClient нет строки "}"
	...
	И я очищаю окно сообщений пользователю
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
		|'Сообщить(УбратьТехническиеДанныеИзСтроки("{ВнешняяОбработка.РаботаСИнтерфейсомДополнительный.Форма.Форма.Форма(386)}: {ВнешняяОбработка.РаботаСИнтерфейсомДополнительный.Форма.Форма.Форма(368)}: {ВнешняяОбработка.VanessaAutomation.МодульОбъекта(4701)}: [Failed] Таблицы должны совпадать, а они различны."))'|
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 1
	Тогда не появилось окно предупреждения системы
	Тогда в логе сообщений TestClient есть строки:
		|'[Failed] Таблицы должны совпадать, а они различны.'|
...
Сценарий: Вызов некоторых процедур напрямую. ОбработкаОповещения. СТекущегоШага.
	...
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
		...
		|'ДанныеОповещения.Вставить("НомерСтрокиДляЗапускаСТекущегоШагаСценария",16);'|
...
Сценарий: Вызов некоторых процедур напрямую. Прочие процедуры.
	...
	И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
		| 'ЭтоLinux = Истина;'                                                                                             |
		| 'Если НЕ ПроверитьСтрокуRegExpLinux("111", "1*1") Тогда ВызватьИсключение "Ошибка1"; КонецЕсли;'                 |
		| 'Если ПроверитьСтрокуRegExpLinux("222", "1*1") Тогда ВызватьИсключение "Ошибка2"; КонецЕсли;'                    |
		| 'Если НЕ СтрокаСоответствуетШаблону("<222&quot;>", "<2*2&quot;>") Тогда ВызватьИсключение "Ошибка3"; КонецЕсли;' |
		| 'ЭтоLinux = ЭтоLinux();'                                                                                         |
		| 'Сообщить("ЭтоLinux=" + ЭтоLinux);'                                                                                            |
	И я нажимаю на кнопку с именем 'ВыполнитьПроизвольныйКод'
	И пауза 2
	Тогда не появилось окно предупреждения системы
	Тогда в логе сообщений TestClient есть строки:
