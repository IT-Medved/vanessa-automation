...
Сценарий: Редактирование таблицы Gherkin
	...
	* Затем я записываю действия пользователя чтобы там была таблица
		...
		И элемент формы с именем "Статистика" стал равен '1/1/16, 16/0/0'
		И Пауза 1
	...
	* Затем я вызываю редактор таблицы
		...
		И в поле с именем 'РеквизитПроизвольныйКод' я ввожу текст
			...
			| ' //  Сообщить(Стр);                                                                        '         |
			| ' Если Лев(СокрЛП(Стр),1) = "\|" И Найти(Стр, "\'N\'") > 0 Тогда                            '         |
			| '    Элементы.СгенерированныйСценарий.УстановитьГраницыВыделения(НомерСтроки+1,1,НомерСтроки+1,2); '  |
			| '    Прервать;                                                                            '           |
			| '  КонецЕсли;                                                                             '           |
			| ' КонецЦикла;                                                                             '           |
	...
	* Снять и установить все пометки			
		И в таблице "СписокКолонок" я нажимаю на кнопку с именем 'РедакторТаблицСписокКолонокСнятьВсе'	
		Тогда таблица "СписокКолонок" стала равной:
			| 'Пометка' | 'Значение' |
			| 'Нет'     | 'N'        |

		И в таблице "СписокКолонок" я нажимаю на кнопку с именем 'РедакторТаблицСписокКолонокПометитьВсе'		
		Тогда таблица "СписокКолонок" стала равной:
			| 'Пометка' | 'Значение' |
			| 'Да'      | 'N'        |
