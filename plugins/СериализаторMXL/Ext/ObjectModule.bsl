	...
	Для каждого Элем Из Данные Цикл
		...
		ИначеЕсли Найти(Стр, ЗначениеСоответствияРегистрБухгалтерии) > 0 Тогда
			...
			Если ВозвращатьНаборЗаписей Тогда
				...
				Данные[Элем.Ключ] = Результат;
			Иначе
				Данные[Элем.Ключ] = ПолучитьКлючиЗаписиРБПоНаборуЗаписейРБ(Элем.Значение, ИмяРегистра);
			КонецЕсли;			
		КонецЕсли;
...
Функция ВычислитьЗначениеПоТипуПоля(НаборМдРеквизитов,стрПоле,ВызыватьОшибкуНеЗаполненногоЗначения = Ложь)
	...
	Попытка
		...
		Если НЕ УжеВычислилиЗначение и ЕстьМетаданныеРеквизита Тогда
			...
			ИначеЕсли Найти(ПолноеИмяТипаРеквизита, ВернутьЗначениеСоответствия("ПланОбмена")) = 1 Тогда
				менеджерОбъекта = ПланыОбмена[МетаданныеТипаРеквизита.Имя];
			КонецЕсли;
...
// Функция определяет имеет ли переданный объект метаданных предопределенные значения
//
// Возврат - Истина, если переданный объект метаданных может иметь предопределенные значения, Ложь - противном случае
Функция ОбъектМетаданныхИмеетПредопределенныеЗначения(ОбъектМД) Экспорт
	...
	Если Метаданные.Справочники.Содержит(ОбъектМД)
		ИЛИ Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМД)
		ИЛИ Метаданные.ПланыСчетов.Содержит(ОбъектМД)
		ИЛИ Метаданные.ПланыВидовРасчета.Содержит(ОбъектМД) Тогда
	...
	Для каждого РеквизитМд Из МдРеквизиты Цикл
		...
		Иначе
			Если КорневойТипКонфигурации = "РегистрБухгалтерии" Тогда
				Если ИмяРеквизита = "Счет" ИЛИ ИмяРеквизита = "СчетДт" ИЛИ ИмяРеквизита = "СчетКт" Тогда
...
Функция ДобавитьРеквизитОбъектаВВыгрузку(ДанныеДляВыгрузки, МассивДанных, Объект, РеквизитМд, ЗначениеРеквизита, Мд, ШапкаДанных)
	...
	// Не выгружаем ссылку для предопределенных элементов.
	// Используем ленивую проверку, сначала проверяем имя реквизита,
	// далее есть ли у метаданных объекта предопределенные значения,
	// и только потом свойство Предопределенный.
	// 
	Если ЗаписьВыгрузки.Реквизит = "Ссылка" И ОбъектМетаданныхИмеетПредопределенныеЗначения(Мд) И Объект.Предопределенный Тогда
		Возврат Истина;
	КонецЕсли;
