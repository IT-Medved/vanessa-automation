...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("MCPРежимЧтенияФормы") Тогда
		MCPРежимЧтенияФормы = Параметры.MCPРежимЧтенияФормы;
	Иначе
		MCPРежимЧтенияФормы = Ложь;
	КонецЕсли;
	
	MCPФорматЧтенияФормы = "";
	Если Параметры.Свойство("MCPФорматЧтенияФормы") Тогда
		MCPФорматЧтенияФормы = Параметры.MCPФорматЧтенияФормы;
	КонецЕсли;	
...
Процедура ЗаполнитьСписокКартинокЭлементовФормы()
	
	ТабдокМакет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("ИконкиЭлементовФормы"); 
	НомерСтр = 1;  
	Пока НомерСтр <= ТабдокМакет.ВысотаТаблицы Цикл 
		ТекущаяОбласть = ТабдокМакет.Область("R" + Строка(НомерСтр) + "C1");
		ИконкиЭлементовФормы.Добавить(ТекущаяОбласть.Текст,,, ТекущаяОбласть.Картинка); 
		НомерСтр = НомерСтр + 1; 
	КонецЦикла;
...
Процедура ДобавитьКонтролВДерево(ТекущийКонтрол,ЭлементДерева)
	...
	ИначеЕсли Тип = Тип("ТестируемаяФорма") Тогда
		...
		ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("Форма").Картинка;
	...
	Попытка
		...
		Если ТипЗнч(ТекущийКонтрол) = Тип("ТестируемаяТаблицаФормы") Тогда
			ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("Таблица").Картинка;	
		Иначе
			Если ТекущийКонтрол.Вид = ВидГруппыФормы.КоманднаяПанель Тогда 
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("КоманднаяПанель").Картинка;		
			ИначеЕсли ТекущийКонтрол.Вид = ВидДополненияЭлементаФормы.ОтображениеСостоянияПросмотра Тогда
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("СостояниеПросмотра").Картинка;
			ИначеЕсли ТекущийКонтрол.Вид = ВидДополненияЭлементаФормы.ОтображениеСтрокиПоиска Тогда
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("СтрокаПоиска").Картинка;
			ИначеЕсли ТекущийКонтрол.Вид = ВидДополненияЭлементаФормы.УправлениеПоиском Тогда
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("УправлениеПоиском").Картинка;
			ИначеЕсли  ТипЗнч(ТекущийКонтрол.Вид) = Тип("ВидГруппыФормы") Тогда			
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("Группа").Картинка;	
			ИначеЕсли ТипЗнч(ТекущийКонтрол.Вид) = Тип("ВидКнопкиФормы") Тогда
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("Кнопка").Картинка;
			ИначеЕсли  ТипЗнч(ТекущийКонтрол.Вид) = Тип("ВидПоляФормы") Тогда 
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("Поле").Картинка;
			ИначеЕсли  ТипЗнч(ТекущийКонтрол.Вид) = Тип("ВидДекорацииФормы") Тогда 
				ЭлементДерева.Картинка = ИконкиЭлементовФормы.НайтиПоЗначению("Декорация").Картинка;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьДанныеФормыРекурсивно(ТекущийКонтрол,ТекЭлементДерева, ДобавленоНеГрупп)
	СтрокиДерева = ТекЭлементДерева.ПолучитьЭлементы();
	НовыйЭлементДерева = СтрокиДерева.Добавить();
	ДобавитьКонтролВДерево(ТекущийКонтрол,НовыйЭлементДерева);
	Если ТипЗнч(ТекущийКонтрол) <> Тип("ТестируемаяГруппаФормы") Тогда
		ДобавленоНеГрупп = ДобавленоНеГрупп + 1;
	КонецЕсли;	
	...
	ТекДобавленоНеГрупп = 0;
	Для Каждого Элем Из МассивЭлементов Цикл
		Если НЕ НастройкамиПереданоДобавлятьТакойЭлемент(Элем) Тогда
			Продолжить;
		КонецЕсли;
		ЗаполнитьДанныеФормыРекурсивно(Элем,НовыйЭлементДерева, ТекДобавленоНеГрупп);
	КонецЦикла;
	
	Если MCPФорматЧтенияФормы = "buttons" И ТипЗнч(ТекущийКонтрол) = Тип("ТестируемаяГруппаФормы") И ТекДобавленоНеГрупп = 0 Тогда
		СтрокиДерева.Удалить(СтрокиДерева.Количество() - 1);
	КонецЕсли;	
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьДанныеФормыЛинейно(ТекущийКонтрол,ТекЭлементДерева)
	НовыйЭлементДерева = ТекЭлементДерева.ПолучитьЭлементы().Добавить();
	ДобавитьКонтролВДерево(ТекущийКонтрол,НовыйЭлементДерева);
...
Процедура ЗаполнитьДеревоФормы(ЗаполнятьДеревоФормы)
	...
	Если ЗаполнятьДеревоФормы Тогда
		Если СтроитьДерево Тогда
			ДобавленоНеГрупп = 0;
			ЗаполнитьДанныеФормыРекурсивно(ТекущееОкно,ДеревоФормы, ДобавленоНеГрупп);
		Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	СкрытьДопКолонки();
	Ванесса = ВладелецФормы;
	Если MCPРежимЧтенияФормы Тогда
		ПолучатьАктивныйЭлементИзTestClient = Ложь;
		// Включаем получение видимости элементов
	...
	Если MCPРежимЧтенияФормы Тогда
		ПодключитьОбработчикОжидания("MCPЗавершитьЧтениеФормы", 1.2, Истина);
	Иначе
...
&НаКлиенте
Процедура MCPЗавершитьЧтениеФормы() Экспорт
	...
	MCPДанныеЧтенияФормы = MCPСформироватьРезультатЧтенияФормы();
	Закрыть(MCPДанныеЧтенияФормы);
	
КонецПроцедуры

&НаКлиенте
Функция MCPСформироватьРезультатЧтенияФормы()
	
	Строки = Новый Массив;
	MCPДобавитьСтрокиДереваФормы(ДеревоФормы.ПолучитьЭлементы(), 0, Строки);
	ДанныеДерева = СтрСоединить(Строки, Символы.ПС);
	
	Результат = Новый Структура;
	Результат.Вставить("treeText", ДанныеДерева);
	Результат.Вставить("stepText", ТекстШагов.ПолучитьТекст());
	Возврат Результат;
	
КонецФункции

&НаКлиенте
Процедура MCPДобавитьСтрокиДереваФормы(ЭлементыДерева, Уровень, Строки)
	
	Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
		Если ЭлементДерева.ТипСтрока = "Тестируемое окно клиентского приложения" Тогда
			ТекВидимостьЭлемента = Истина;
		ИначеЕсли ЭлементДерева.ТипСтрока = "Тестируемая форма" Тогда
			ТекВидимостьЭлемента = Истина;
		Иначе
			ТекВидимостьЭлемента = ЭлементДерева.ТекущаяВидимость;
		КонецЕсли;
		
		Строки.Добавить(Лев("------------------------------", Уровень) + "- " + ЭлементДерева.Синоним + " [" + ЭлементДерева.Имя + "] {" + ЭлементДерева.ТипСтрока + "} {" + Ванесса.Локализовать("Видимость") + ":" + ТекВидимостьЭлемента + "}");
		MCPДобавитьСтрокиДереваФормы(ЭлементДерева.ПолучитьЭлементы(), Уровень + 1, Строки);
	КонецЦикла;
...
Процедура ЗаполнитьТекстШаговПоАктивномуЭлементу()
	...
	Если ТипЗнч(ЭлементФормы) = Тип("ТестируемоеПолеФормы") Тогда
		...
		Если Вид = ВидПоляФормы.ПолеВвода Тогда
			Если ТаблицаВладелец = Неопределено Тогда
				Попытка
					Если ЭлементФормы.Вид = ВидПоляФормы.ПолеВвода Тогда
						...
						Попытка            
							ЗначениеПоля = ПолучитьТекстРедактированияЭлементаФормы(ЭлементФормы);
				...
				Попытка
					Если ТекстРедактирования = Неопределено Тогда
						ТекстРедактирования = ПолучитьТекстРедактированияЭлементаФормы(ЭлементФормы);
					КонецЕсли;	 
			...
			Иначе	
				Попытка          
					ЗначениеПоля = ПолучитьТекстЯчейкиТаблицы(ЭлементТаблица, ЭлементФормы);
					МассивСтрок  = Ванесса.РазложитьСтрокуВМассивПодстрокКлиент(ЗначениеПоля,Символы.ПС);
					Если МассивСтрок.Количество() <=1 Тогда
		...
		ИначеЕсли Вид = ВидПоляФормы.ПолеТабличногоДокумента Тогда
			...
			Попытка
				...
				КонецЕсли;	 
				МассивШагов.Добавить("");
				МассивШагов.Добавить("И я жду когда в табличном документе '" + ЭлементФормы.Имя + "' заполнится ячейка '" + АдресТекущейОбласти + "' в течение 10 секунд");
				...
				МассивШагов.Добавить("И я жду, что в табличном документе '" + ЭлементФормы.Имя + "' ячейка '" + АдресТекущейОбласти + "' станет равна '" + ЭкранироватьСпецСимволы(ТекстТекущейОбласти) + "'" + " в течение 10 секунд");
		...
		ИначеЕсли Вид = ВидПоляФормы.ПолеНадписи Тогда
			Если ТаблицаВладелец = Неопределено Тогда
				...
				ПредставленияГиперссылок = Неопределено;
				Если Ванесса.Версия8325ИлиВыше Тогда
					Попытка
						ПредставленияГиперссылок = ЭлементФормы.ПолучитьПредставленияГиперссылокФорматированнойСтроки();
					Исключение
					КонецПопытки;
				КонецЕсли;
				
				Если ПредставленияГиперссылок = Неопределено ИЛИ ПредставленияГиперссылок.Количество() = 0 Тогда
				...
				Иначе	
					Для Каждого ТекПредставленияГиперссылок Из ПредставленияГиперссылок Цикл
						МассивШагов.Добавить("И у поля '" + ЭкранироватьСпецСимволы(ЭлементФормы.ТекстЗаголовка) + "' я нажимаю гиперссылку '" + ЭкранироватьСпецСимволы(ТекПредставленияГиперссылок) + "'");
						МассивШагов.Добавить("И у поля с именем '" + ЭлементФормы.Имя + "' я нажимаю гиперссылку '" + ЭкранироватьСпецСимволы(ТекПредставленияГиперссылок) + "'");
					КонецЦикла;	
				КонецЕсли;	
		...
		Если ТаблицаВладелец <> Неопределено Тогда
			...
			МассивШагов.Добавить("");
			МассивШагов.Добавить("И я жду, что в таблице '" + ТаблицаВладелец.Имя + "' количество строк будет ""больше"" 0 в течение 10 секунд");
	...
	ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ТестируемаяГруппаФормы") Тогда	
		МассивШагов.Добавить("И я жду появления элемента '" + ЭкранироватьСпецСимволы(ЭлементФормы.ТекстЗаголовка) + "' в течение 10 секунд");
		МассивШагов.Добавить("И я жду появления элемента с именем '" + ЭлементФормы.Имя + "' в течение 10 секунд");
	КонецЕсли;	 
...
&НаКлиенте
Функция ПолучитьТекстРедактированияЭлементаФормы(ПолеРеквизит)
...
&НаКлиенте
Функция ПолучитьТекстЯчейкиТаблицы(ЭлементТаблица, ЭлементФормы)
	Если Ванесса.Версия8323ИлиВыше Тогда
		ТекстЯчейки = ЭлементТаблица.ПолучитьТекстЯчейки(ЭлементФормы.Имя);
		Если НЕ ЗначениеЗаполнено(ТекстЯчейки) Тогда 
			Попытка 
				ТекстЯчейки = ЭлементТаблица.ПолучитьТекущийЭлемент().ПолучитьПредставлениеДанных();
				
				Если НЕ ЗначениеЗаполнено(ТекстЯчейки) Тогда
					Возврат ЭлементТаблица.ПолучитьТекущийЭлемент().ПолучитьОтображаемыйТекст();
				КонецЕсли;	
				
				Возврат ЭлементТаблица.ПолучитьТекущийЭлемент().ПолучитьПредставлениеДанных();
			Исключение
				Возврат ТекстЯчейки;
			КонецПопытки;
		Иначе
		...
		КонецЕсли;	
	Иначе	
		Возврат ЭлементТаблица.ПолучитьТекстЯчейки(ЭлементФормы.Имя);
	КонецЕсли;
КонецФункции	

&НаКлиенте
Функция НастройкамиПереданоДобавлятьТакойЭлемент(Элем)
	
	Если MCPФорматЧтенияФормы = "buttons" Тогда
		Если ТипЗнч(Элем) = Тип("ТестируемаяКнопкаФормы")
			ИЛИ ТипЗнч(Элем) = Тип("ТестируемаяГруппаФормы")
			ИЛИ ТипЗнч(Элем) = Тип("ТестируемыйКомандныйИнтерфейсОкна")
			ИЛИ ТипЗнч(Элем) = Тип("ТестируемаяФорма")
			Тогда
				Возврат Истина;
			Иначе
				Возврат Ложь;
