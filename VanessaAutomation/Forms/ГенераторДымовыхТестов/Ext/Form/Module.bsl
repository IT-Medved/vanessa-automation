...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТолькоИзмененныеОтносительноКонфигурацииПоставщика") Тогда
		ТолькоИзмененныеОтносительноКонфигурацииПоставщика = Параметры.ТолькоИзмененныеОтносительноКонфигурацииПоставщика;
	КонецЕсли;
	...
	Элементы.ИмяКонфигурацииПоставщика.Доступность = ТолькоИзмененныеОтносительноКонфигурацииПоставщика;
...
Процедура ПередЗакрытиемПослеВопроса(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ПараметрыФормы.Вставить("ДымовыеТестыТолькоИзмененныеОтносительноКонфигурацииПоставщика", ТолькоИзмененныеОтносительноКонфигурацииПоставщика);
...
&НаКлиенте
Процедура ТолькоИзмененныеОтносительноКонфигурацииПоставщикаПриИзменении(Элемент)
...
Процедура СохранитьНастройкиСценариевВФайл(Команда)
	...
	Состояние(Локализовать("Выполняется выгрузка настройки сценариев. Пожалуйста, подождите..."));
...
Процедура СформироватьФайлыСценариевТестирования()
	...
	ТекстСравнения = "";
	Если ТолькоИзмененныеОтносительноКонфигурацииПоставщика Тогда
...
Процедура СформироватьСценарииПоМетаданным(Команда)
	...
	СформироватьСценарииПоМетаданнымКлиент();
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьСценарииПоОтсутствующимТестам(Команда)
	...
	ТолькоПоОтсутствующимТестам = Истина;
	СформироватьСценарииПоМетаданнымКлиент(ТолькоПоОтсутствующимТестам);
...
&НаКлиенте
Процедура СформироватьСценарииПоМетаданнымКлиент(ТолькоПоОтсутствующимТестам = Ложь)
	...
	Иначе
		...
		// Заполняем признак наличия теста в фича-файлах
		ЗаполнитьПризнакНаличияТестов();
		
		// Если нужно показать только отсутствующие тесты
		Если ТолькоПоОтсутствующимТестам Тогда
			ОставитьТолькоОтсутствующиеТесты();
		КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьПризнакНаличияТестов()
	
	// Кэшируем содержимое всех фича-файлов для оптимизации
	КэшФичаФайлов = Новый Соответствие;
	
	Для Каждого ТекСтрока Из ТаблицаКодаСценариев1С Цикл
		
		ТекСтрока.ЕстьТест = Ложь;
		
		ПутьКФайлу = ТекСтрока.ПутьКФичаФайлу;
		Если Не ЗначениеЗаполнено(ПутьКФайлу) Тогда
			Продолжить;
		КонецЕсли;
		
		// Получаем содержимое из кэша или читаем файл
		Если КэшФичаФайлов.Получить(ПутьКФайлу) = Неопределено Тогда
			
			Файл = Новый Файл(ПутьКФайлу);
			Если Не Файл.Существует() Тогда
				КэшФичаФайлов.Вставить(ПутьКФайлу, Неопределено);
				Продолжить;
			КонецЕсли;
			...
			Попытка
				...
				КэшФичаФайлов.Вставить(ПутьКФайлу, Текст.ПолучитьТекст());
			Исключение
				КэшФичаФайлов.Вставить(ПутьКФайлу, Неопределено);
				Продолжить;
			КонецПопытки;
		...
		ТекстФайла = КэшФичаФайлов.Получить(ПутьКФайлу);
		Если ТекстФайла = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		// Проверяем наличие сценария
		МассивЗаголовков = МассивЗаголовковСценариевИзСтроки(ТекСтрока.ТекстКодСценария);
		
		Если СценарийУжеЕстьВФайле(ТекстФайла, МассивЗаголовков) Тогда
			ТекСтрока.ЕстьТест = Истина;
		КонецЕсли;
...
&НаКлиенте
Процедура ОставитьТолькоОтсутствующиеТесты()
	
	Индекс = 0;
	Пока Индекс < ТаблицаКодаСценариев1С.Количество() Цикл
		СтрокаТаблицы = ТаблицаКодаСценариев1С.Получить(Индекс);
		Если СтрокаТаблицы.ЕстьТест Тогда
			ТаблицаКодаСценариев1С.Удалить(Индекс);
		Иначе
			Индекс = Индекс + 1;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодготовкаДанныхКлиент()
	...
	// Создаем индекс для быстрого поиска: FullName -> Идентификатор
	ИндексМетаданных = Новый Соответствие;
	ЭлементыДереваПодготовкиДанных = МодульПодготовкаИЗагрузкаДанных.MetadataList.ПолучитьЭлементы();
	ПостроитьИндексДереваРекурсией(ЭлементыДереваПодготовкиДанных, ИндексМетаданных);
	...
	Для Каждого ЭлементКласс Из ЭлементыДерева Цикл
		...
		Для Каждого ЭлементКласса Из ЭлементыКласса Цикл
			...
			// Быстрый поиск по индексу вместо рекурсии
			НайденныйИдентификатор = ИндексМетаданных.Получить(ИмяИсточникаДляПоиска);
	...
	// Устанавливаем флаг для автоматической генерации фичи при открытии
	МодульПодготовкаИЗагрузкаДанных.AutoGenerateFeature = Истина;
...
// Построение индекса дерева метаданных для быстрого поиска
//
&НаКлиенте
Процедура ПостроитьИндексДереваРекурсией(ЭлементыДерева, ИндексМетаданных)
	
	Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
		
		// Добавляем элемент в индекс по FullName
		ПолноеИмя = ЭлементДерева.FullName;
		Если ЗначениеЗаполнено(ПолноеИмя) Тогда
			ИндексМетаданных.Вставить(ПолноеИмя, ЭлементДерева.ПолучитьИдентификатор());
		КонецЕсли;
		
		// Рекурсивно обрабатываем дочерние элементы
		ПостроитьИндексДереваРекурсией(ЭлементДерева.ПолучитьЭлементы(), ИндексМетаданных);
...
Процедура ДобавитьДанныеОбъектаМетаданных(Параметры)
	...
	ИначеЕсли Параметры.ВидОбъектаМетаданных = "РегистрыРасчета" Тогда
		...
		Параметры.Вставить("ВидОбъектаРП", "регистра расчета");
		Параметры.Вставить("ИмяФормыОбъекта", "");
		Параметры.Вставить("ИмяФормыСписка", "ОсновнаяФормаСписка");
