...
// Запись документа по навигационной ссылке
//
// Параметры:
...
//
&НаСервере
Функция РезультатЗаписиДокументаПоНавигационнойСсылке(Знач НавигационнаяСсылка, Знач Режим, Знач ВызыватьИсключение = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Ссылка) И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда 
		...
		Иначе
			РежимЗначение = Режим;
		КонецЕсли; 
		Результат = РезультатЗаписиДокументаНаСервере(Ссылка, РежимЗначение, ВызыватьИсключение);
...
// 	ПомещатьВХранилище - Булево - для констант типа "ХранилищеЗначения"
...
Процедура УстановитьКонстанту(ИмяКонстанты, ЗначениеКонстанты,
	ПомещатьВХранилище = Ложь, УстанавливатьПараметрСеанса = Ложь) Экспорт
	
	СохранитьЗначениеКонстанты(ИмяКонстанты, ПомещатьВХранилище, УстанавливатьПараметрСеанса);	
	УстановитьКонстантуНаСервере(ИмяКонстанты, ЗначениеКонстанты, ПомещатьВХранилище, УстанавливатьПараметрСеанса);
...
Процедура УстановитьКонстантуНаСервере(Знач ИмяКонстанты, Знач ЗначениеКонстанты, 
	ПомещатьВХранилище, УстанавливатьПараметрСеанса)
...
Процедура ОткатитьКонстанты(КэшКонстант)
	...
	Для каждого КлючИЗначение Из КэшКонстант Цикл
		...
		УстановитьКонстантуНаСервере(ИмяКонстанты
			...
			, ОписаниеЗначения.ПомещатьВХранилище
...
&НаКлиенте
Процедура СохранитьЗначениеКонстанты(ИмяКонстанты, ПомещатьВХранилище, УстанавливатьПараметрСеанса)
	...
	ОписаниеЗначения.Адрес = VAЗначениеКонстанты(ИмяКонстанты, ПомещатьВХранилище, Истина);
	ОписаниеЗначения.ПомещатьВХранилище = ПомещатьВХранилище;
...
Функция НовоеОписаниеЗначенияКонстанты()
	...
	Результат.Вставить("ПомещатьВХранилище", Ложь);
