...
Процедура ПриОткрытии(Отказ)
	...
	ДопПараметры.Вставить("ДанныеДляВставкиГиперссылокВОписаниеШагов",Ванесса.ДанныеДляВставкиГиперссылокВОписаниеШагов());
...
// Возвращает json строку во всеми доступными шагами
&НаКлиенте
Функция ВсеДоступныеШагиMCP() Экспорт
	МассивДляВыгрузкиШагов = ПодготовитьТаблицуДляВыгрузкиШагов();
	Возврат Ванесса.ЗаписатьОбъектJSON(МассивДляВыгрузкиШагов);
КонецФункции	

// Возвращает строку json с шагами, которые были найдены по строке поиска
// Пример строки поиска: кнопка|кнопки|кнопку
&НаКлиенте
Функция ПоискШаговПоСтроке(Знач СтрокаПоиска, Знач СтрокаИсключение) Экспорт
	
	ДвДанныеvbFeatureReader = ПолучитьДДFeatureReader();
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("ДанныеДляВставкиГиперссылокВОписаниеШагов",Ванесса.ДанныеДляВставкиГиперссылокВОписаниеШагов());
	ДопПараметры.Вставить("ДвДанныеvbFeatureReader", ДвДанныеvbFeatureReader);
	ДопПараметры.Вставить("НужноВернутьШагиПоОтбору", Истина);
	МассивСтрокПоиск = Ванесса._СтрРазделить(СтрокаПоиска, "|");
	
	Если ЗначениеЗаполнено(СтрокаИсключение) Тогда
		МассивСтрокИсключение = Ванесса._СтрРазделить(СтрокаИсключение, "|");
		Для Сч = 0 По МассивСтрокИсключение.Количество() - 1 Цикл
			МассивСтрокИсключение[Сч] = НРег(МассивСтрокИсключение[Сч]);
		КонецЦикла;
	Иначе
		МассивСтрокИсключение = Новый Массив;
	КонецЕсли;	
	
	ДопПараметры.Вставить("МассивСтрокИсключение", МассивСтрокИсключение);
	Результат = ПоискШаговПоСтрокеСервер(МассивСтрокПоиск, ДопПараметры);
	
	Возврат Ванесса.ЗаписатьОбъектJSON(Результат);
...
Процедура MarkdownViewerДокументСформирован(Элемент)
	...
	Исключение
		Сообщить("Не работает отображение описаний в формате Markdown");
		ИспользоватьMarkdownViewer = Ложь;
		УстановитьВидимостьОписанийШагов(ЭтаФорма);
	КонецПопытки;
...
&НаКлиенте
Процедура MarkdownViewerПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	
	Если ТипЗнч(ДанныеСобытия) = Тип("Структура") ИЛИ ТипЗнч(ДанныеСобытия) = Тип("ФиксированнаяСтруктура") Тогда
		Если ДанныеСобытия.Свойство("Href") и ЗначениеЗаполнено(ДанныеСобытия.Href) Тогда
			Если НРег(Лев(СокрЛП(ДанныеСобытия.Href), 4)) = "http" Тогда
				НачатьЗапускПриложения(Ванесса.ОписаниеПустогоОбработчика(2), ДанныеСобытия.Href);
...
Процедура ДеревоШаговИмяФайлаПолеОткрытие(Элемент, СтандартнаяОбработка)
	...
	НачатьЗапускПриложения(ОписаниеОповещения, СвойстваФайла.Путь);
...
Процедура УстановитьВидимостьОписанийШагов(Форма)
	...
	ЭлементыФормы.ДеревоШаговОписаниеШага.Видимость               = НЕ Форма.ИспользоватьMarkdownViewer;
	ЭлементыФормы.MarkdownViewer.Видимость                        = Форма.ИспользоватьMarkdownViewer;
...
Функция ДобавитьШаги(Дерево,Тип,Данные,ТаблицаПеревода,СтрТаблицаИзвестныхStepDefinition,ДопПараметры)
	...
	СтрокаШага.ОписаниеШага       = ВставитьГиперссылкиВОписаниеШага(Данные.ОписаниеШага, ДопПараметры);
...
Процедура ЗаполнитьДеревоИзвестныхШаговНаСервере(ДвДанныеvbFeatureReader,ДопПараметры)
	...
	НужноВернутьШагиПоОтбору = Ложь;
	Если ДопПараметры.Свойство("НужноВернутьШагиПоОтбору") Тогда
		НужноВернутьШагиПоОтбору = ДопПараметры.НужноВернутьШагиПоОтбору;
	КонецЕсли;	
	...
	Для Каждого СтрТаблицаИзвестныхStepDefinition Из ТабРезультат Цикл
		...
		Если НЕ ПоказыватьУстаревшиеШаги Тогда
			Если Найти(НРег(СтрТаблицаИзвестныхStepDefinition.ТипШага),"устаревшее") > 0 Тогда
				Продолжить;
			КонецЕсли;	 
		КонецЕсли;	 
	...
	Для Каждого СтрТаблицаИзвестныхStepDefinition Из ТабРезультат Цикл
		...
		Если НЕ ПоказыватьУстаревшиеШаги Тогда
			Если Найти(НРег(Данные.ТипШага),"устаревшее") > 0 Тогда
				Продолжить;
			КонецЕсли;	 
		КонецЕсли;	 
		
		СтрокаШага = ДобавитьШаги(Дерево,Тип,Данные,ТаблицаПеревода,СтрТаблицаИзвестныхStepDefinition,ДопПараметры);
		Если НужноВернутьШагиПоОтбору Тогда
			НадоДобавитьШагВВыдачу = Истина;
			Если ДопПараметры.МассивСтрокИсключение.Количество() > 0 Тогда
				Для Каждого СтрокаИсключение Из ДопПараметры.МассивСтрокИсключение Цикл
					Если Найти(НРег(СтрокаШага.ПредставлениеТеста), СтрокаИсключение) > 0 Тогда
						НадоДобавитьШагВВыдачу = Ложь;
						Прервать;
					КонецЕсли;	
					Если Найти(НРег(СтрокаШага.ОписаниеШага), СтрокаИсключение) > 0 Тогда
						НадоДобавитьШагВВыдачу = Ложь;
						Прервать;
					КонецЕсли;	
					Если Найти(НРег(Тип), СтрокаИсключение) > 0 Тогда
						НадоДобавитьШагВВыдачу = Ложь;
						Прервать;
					КонецЕсли;	
				КонецЦикла;	
			...
			Если НадоДобавитьШагВВыдачу И ДопПараметры.КешШагов[СтрокаШага.Снипет] = Неопределено Тогда
				ДанныеШага = Новый Структура;
				ДанныеШага.Вставить("Шаг", СтрокаШага.ПредставлениеТеста);
				ДанныеШага.Вставить("Описание", СтрокаШага.ОписаниеШага);
				ДанныеШага.Вставить("ТипШага", Тип);
				ДопПараметры.КешШагов.Вставить(СтрокаШага.Снипет, ДанныеШага);
			КонецЕсли;	
		КонецЕсли;	
	КонецЦикла;	
...
&НаСервере
Функция ПоискШаговПоСтрокеСервер(Знач МассивСтрокПоиск, Знач ДопПараметры)
	
	ДвДанныеvbFeatureReader = ДопПараметры.ДвДанныеvbFeatureReader;
	ДопПараметры.Вставить("КешШагов", Новый Соответствие);
	
	Для Сч = 0 По МассивСтрокПоиск.Количество()-1 Цикл
		ТекСтр = МассивСтрокПоиск[Сч];
		ФильтрДереваШагов = ТекСтр;
		ЗаполнитьДеревоИзвестныхШаговНаСервере(ДвДанныеvbFeatureReader, ДопПараметры);
	КонецЦикла;
	
	Результат = Новый Массив;
	Для Каждого Элем Из ДопПараметры.КешШагов Цикл
