...
Процедура СкачатьУроки(Команда)
	...
	ПодключитьОбработчикОжидания("СкачатьУрокиОбработчик", 0.1, Истина);
...
Функция НайденКешОзвучки()
	...
	Если ЗначениеЗаполнено(ПроверяемыйФичаФайл) И
		...
		Если Ванесса.ВерисияКешФайлаОзвучкиСовпадаетСМажорнойВерсией() Тогда
			Элементы.ГруппаКешОзвучки.Видимость = Ложь;
			ПоказатьПредупреждение(Неопределено,
				Ванесса.ПодставитьПараметрыВСтроку(
					Ванесса.Локализовать("Данный урок был изменен после выпуска релиза <%1>. Для работы урока необходимо включить генерацию речи.")
						, Ванесса.МажорнаяРелизнаяВерсия())
			);
			Возврат Ложь;
		Иначе		
			Возврат Ложь;
...
&НаКлиенте
Процедура СкачатьУрокиОбработчик()
...
Функция НомерРелиза()
	...
	Возврат Ванесса.МажорнаяРелизнаяВерсия();
